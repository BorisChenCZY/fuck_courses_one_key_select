...
SERVER_URL = 'http://borischen.me:300'
...
def get_url_courses(sustc, url):
    ...
    # r = sustc.s.post('http://jwxt.sustc.edu.cn/jsxsd', post_data)
    # print(r.content)
    # quit()
    print(sustc.post_website(url, post_data))
...
def get_selected_course_selector(sustc):
    ...
    for tr in trs:
        ...
        # selected_courses_selector.append(course)
...
def dis_select_courses_from_table(self):
    ...
    for selecor in selectors:
        ...
        if info['success']:
            add_log(self, '成功退选：{} '.format(search(self.courses['all'], selecor)[0].course_name))
        else:
            add_log(self, '{} {}'.format(search(self.courses['all'], selecor)[0].course_name, info['message']))
...
def fuck_courses(self):
    ...
    for selecor in selectors:
        ...
        info = (self, try_to_select(self, selecor))[1]
        info = (json.loads(info))
        if info['success']:
            add_log(self, '成功选上：{} '.format(find(self.courses['all'], selecor)[0].course_name))
...
def show_courses(self, key, label):
    ...
    # add_log(self, 'show {}'.format(label))
...
def add_log(self, information):
    self.log_textEdit.append(information)
